import { Scheduler,WidgetHelper,AjaxHelper,DomHelper,Popup,Tooltip,Toast,BrowserHelper,StringHelper,LocaleManager,Localizable,DataGenerator,Fullscreen,Events,GlobalEvents,VersionHelper } from '../../build/scheduler.module.js';
...
class Shared extends Localizable(Events()) {
    ...
    constructor() {
        ...
        const overrideData = document.location.href.match(/(?:\?|&)data=([^&]*)/);
        if (overrideData) {
            let parts = overrideData[1].split(',');
            if (parts.length === 1) {
                DataGenerator.overrideData = parseInt(parts[0]);
            }
            else {
                DataGenerator.overrideData = parts.map(p => parseInt(p));
            }
        }
    ...
    applyTheme(theme, initial = false) {
        ...
        link.id = 'theme-css';
        link.rel = 'stylesheet';
        let url = `../build/${productName}.${theme.toLowerCase()}.css`.toLowerCase();
    ...
    // region RootCause
    initRootCause() {
        const isOnline = location.href.includes('bryntum.com'),
            isExcluded = location.href.includes('csp'),
    ...
    startRootCause() {
        let appIds      = {
            ...
            isOnline    = location.href.includes('bryntum.com'),                   // TODO update when we know what the URL will look like online
